async function getCurrency () {
    try {
        const response = await fetch(`https://v6.exchangerate-api.com/v6/2eb0a1f11133b40014c9f277/codes`)
        if(response.ok) {
...
function addToDD (rates) {
    for (let rate of rates) {
        ...
        for (let element of dataListElement) {
            ...
            const optionText = document.createTextNode(rate[1]);

            optionElement.setAttribute('value', rate[1]); 
...
async function makeConversion(event) {
    ...
    const amountInout = amount.value;

    try {
        const response = await fetch(`https://v6.exchangerate-api.com/v6/2eb0a1f11133b40014c9f277/pair/${fromInput}/${toInput}`)
        if(response.ok) {
